...
(defn plotly-html [spec data layout]
  (hiccup/html
   [:html
    [:head
     [:meta {:charset "UTF-8"}]
     [:meta {:name "viewport" :content "width=device-width, initial-scale=1"}]
     (page/include-libs-hiccup spec [:js :css] [:plotly])]
    [:body
     [:div#myDiv]]
    [:script
...
(defn export-plot! [spec filename data layout]
  (let [ext (fs/extension filename)]
    (with-open [playwright (Playwright/create)
      ...
      (let [page (.newPage browser)]
        (.setContent page (plotly-html spec data layout))
